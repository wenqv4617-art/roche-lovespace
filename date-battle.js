  ...
  apps: [
    {
      ...
      async mount(container, roche) {
        ...
        // 渲染配置页面
        await this.renderSetupView(container, roche);
      },

      // 卸载函数
      async unmount(container, roche) {
        const styleTag = document.getElementById("roche-plugin-date-battle-styles");
      ...
      // 注入局部 CSS 样式
      injectStyles() {
        if (document.getElementById("roche-plugin-date-battle-styles")) return;
        const style = document.createElement("style");
        style.id = "roche-plugin-date-battle-styles";
        style.textContent = `
      ...
      // 渲染配置面板
      async renderSetupView(container, roche) {
        const setupDiv = document.getElementById("db-setup-view");
        this.showLoading("加载人设与设定数据...");
          ...
          <div class="setup-container">
            ...
            <div class="form-row">
              ...
              <div class="form-group">
                ...
                <input type="number" id="db-word-count" value="${cachedConfig.wordCount || 300}" min="50" max="800">
              </div>
            </div>
            ...
            <div class="form-group">
              <label>搭载宿主世界书 (可多选，勾选后将自动引入词条作为设定背景)</label>
        ...
        if (document.getElementById("db-resume-btn")) {
          document.getElementById("db-resume-btn").onclick = async () => {
            await this.startGame(container, roche, true);
          };
        }
        ...
        document.getElementById("db-start-btn").onclick = async () => {
          ...
          await this.startGame(container, roche, false);
        };
      },

      // 开始/继续游戏逻辑
      async startGame(container, roche, isResume) {
        let config = {};
        let history = [];
        ...
        this.showLoading("正在加载角色设定并请求开局环境描述...");

        // 获取选定的 User 人设与 Character 角色
        ...
        // 拼接最核心的 System Prompt，用于约束 AI 角色与环境演变
        const systemPrompt = `
...
`;
        ...
        // 载入游戏主场景视图
        this.renderGameView(container, roche, config, history, systemPrompt, character, userPersona);
      },
      ...
      renderGameView(container, roche, config, history, systemPrompt, character, userPersona) {
        ...
        this.renderHistory(chatContainer, history, charName, userName);
      ...
      renderHistory(chatContainer, history, charName, userName) {
        // 过滤内部的初始化第一回合请求 prompt，不让其在聊天气泡中显示
